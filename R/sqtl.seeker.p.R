...
##' the lowest observed nominal P-value to allow the computation to stop. Default is 1e2. 
##' @param nb.perm.max the maximum number of permutations. Default is 1e3. 
...
##' \item{ld}{a LD estimate for the genomic window (median r2)}
##' \item{best.snp}{the SNP with the lowest nominal P-value}
##' \item{best.nominal.pv}{nominal P-value corresponding to the best SNP}
...
##' \item{runtime}{computation time per gene}
...
sqtl.seeker.p <- function(tre.df, genotype.f, gene.loc, genic.window=5e3, min.nb.ext.scores=1e2, nb.perm.max=1e3, verbose=TRUE){
  ...
  compute.nominal.pv <- function(geno.df, tre.dist, permute = FALSE){
    gower <- function (d.mat) {
      ## d.mat <- as.matrix(d.mat)  # Changed from compFscore
      n <- nrow(d.mat)
      A <- -0.5 * d.mat^2
    ...
    if(length(gr.gene) > 0){
      ...
      if (!is.null(genotype.gene)) {
        ...
        if (any(snps.to.keep == "PASS")) {
          ...
          observed <- dplyr::do(dplyr::group_by(genotype.gene, snpId), compute.nominal.pv(., tre.dist, permute = F))
