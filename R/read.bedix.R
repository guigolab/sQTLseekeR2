...
read.bedix <- function(file, subset.reg=NULL, header=TRUE, as.is = TRUE) {
  ...
  read.chunk <- function(gr){
    bed <- tryCatch(unlist(Rsamtools::scanTabix(file, param = GenomicRanges::reduce(gr)), use.names = FALSE, recursive = FALSE), error = function(e) c())
    if (length(bed) == 0) {
      return(NULL)
    }
    ncol <- length(strsplit(bed[1], "\t")[[1]])
    bed <- matrix(unlist(strsplit(bed, "\t", fixed = TRUE), use.names=FALSE, recursive = FALSE), length(bed), ncol, byrow = TRUE)
